...
class SignInController extends GetxController {
  ...
  final ControladorSesionUsuario controladorSesion = Get.find<ControladorSesionUsuario>();
  ...
  if (serviceReponse.status == 200) {
    ...
    Future.delayed(Duration(seconds: 3), () async {
      try {
        ...
        int usuarioId = serviceReponse.body is Map<String, dynamic>
        ...
        if (usuarioId != -1) {
          ServiceHttpResponse? userDetailsResponse = await service.getUsuarioById(usuarioId);
          if (userDetailsResponse != null && userDetailsResponse.status == 200) {
            // Mapear los detalles del usuario
            var userMap = userDetailsResponse.body is Map<String, dynamic>
            ...
            controladorSesion.iniciarSesion(usuarioResponse.id, usuarioResponse.nombre, usuarioResponse.contrasena, usuarioResponse.email);
